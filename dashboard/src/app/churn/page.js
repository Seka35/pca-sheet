  ...
  return (
    <div style={{ paddingBottom: '64px' }}>
      <div style={{ marginBottom: '32px' }}>
        <h1 style={{ fontSize: '24px', fontWeight: '600', marginBottom: '8px' }}>Churn Analysis</h1>
        <p style={{ color: 'var(--text-secondary)' }}>Understand why and when clients are leaving</p>
      </div>
      ...
      <div className="grid-metrics" style={{ marginBottom: '32px' }}>
        <div className="card" style={{ display: 'flex', alignItems: 'center', gap: '16px', padding: '20px 16px' }}>
          ...
          <div>
            <div style={{ fontSize: '12px', color: 'var(--text-secondary)', marginBottom: '4px' }}>Total Churned</div>
            <div style={{ fontSize: '24px', fontWeight: '600' }}>{totalChurned}</div>
          </div>
        </div>
        <div className="card" style={{ display: 'flex', alignItems: 'center', gap: '16px', padding: '20px 16px' }}>
          <div style={{ width: '40px', height: '40px', borderRadius: '8px', backgroundColor: 'rgba(239, 68, 68, 0.1)', color: '#F87171', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <div>
            <div style={{ fontSize: '12px', color: 'var(--text-secondary)', marginBottom: '4px' }}>This Month</div>
            <div style={{ fontSize: '24px', fontWeight: '600' }}>{latestMonth.churned || 0} <span style={{ fontSize: '14px', fontWeight: '400', color: 'var(--text-secondary)' }}>({latestMonth.rate || 0}%)</span></div>
          </div>
        </div>
        <div className="card" style={{ display: 'flex', alignItems: 'center', gap: '16px', padding: '20px 16px' }}>
          <div style={{ width: '40px', height: '40px', borderRadius: '8px', backgroundColor: 'rgba(167, 139, 250, 0.1)', color: '#A78BFA', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <svg width="20" height="20" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z" /></svg>
          </div>
          <div>
            <div style={{ fontSize: '12px', color: 'var(--text-secondary)', marginBottom: '4px' }}>By Category</div>
            <div style={{ fontSize: '24px', fontWeight: '600' }}>{activeCategories.length}</div>
          </div>
        </div>
        <div className="card" style={{ display: 'flex', alignItems: 'center', gap: '16px', padding: '20px 16px' }}>
          ...
          <div>
            <div style={{ fontSize: '12px', color: 'var(--text-secondary)', marginBottom: '4px' }}>By Tier</div>
            <div style={{ fontSize: '24px', fontWeight: '600' }}>{byTier.filter(t => t.churned_count > 0).length}</div>
          </div>
        </div>
      </div>
      ...
      <div className="flex-mobile-column" style={{ display: 'flex', gap: '24px', marginBottom: '32px' }}>
        ...
        <div className="card" style={{ flex: 2, padding: 0, overflow: 'hidden' }}>
          <div style={{ padding: '20px 24px', borderBottom: '1px solid var(--border-color)' }}>
            <h2 style={{ fontSize: '16px', fontWeight: '600', margin: 0 }}>Churn by Month</h2>
          </div>
              ...
              <tbody>
                {byMonth.map((row, i) => (
                  <tr key={i} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                    <td style={{ padding: '12px 24px', textAlign: 'left', fontWeight: '500' }}>{row.month}</td>
                    <td style={{ padding: '12px 24px', color: 'var(--text-secondary)' }}>{row.presents}</td>
                    <td style={{ padding: '12px 24px', fontWeight: '600', color: '#F87171' }}>{row.churned}</td>
                    <td style={{ padding: '12px 24px', fontWeight: '600', color: row.rate < 5 ? '#34D399' : row.rate < 10 ? '#FBBF24' : '#F87171' }}>{row.rate.toFixed(1)}%</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="card" style={{ flex: 1, padding: 0, overflow: 'hidden' }}>
          <div style={{ padding: '20px 24px', borderBottom: '1px solid var(--border-color)' }}>
            <h2 style={{ fontSize: '16px', fontWeight: '600', margin: 0 }}>By Reason</h2>
          </div>
              ...
              return (
                <div key={cat.category} style={{ marginBottom: '16px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}>
                    <span style={{ fontSize: '13px', fontWeight: '500', color: 'var(--text-primary)' }}>{catInfo.label}</span>
                    <span style={{ fontSize: '13px', fontWeight: '600', color: categoryColors[cat.category] || '#8B9AB0' }}>{cat.count}</span>
                  </div>
          ...
          <div style={{ padding: '20px 24px', borderTop: '1px solid var(--border-color)' }}>
            <h3 style={{ fontSize: '14px', fontWeight: '600', margin: '0 0 16px 0' }}>By Tier</h3>
            {byTier.filter(t => t.churned_count > 0).map(t => (
              <div key={t.tier} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
                <span style={{ color: tierColors[t.tier] || '#8B9AB0', fontSize: '11px', fontWeight: '600', border: `1px solid ${tierColors[t.tier]}50`, padding: '2px 8px', borderRadius: '4px' }}>{t.tier}</span>
                <span style={{ fontSize: '13px', fontWeight: '600', color: 'var(--text-primary)' }}>{t.churned_count}</span>
              </div>
            ))}
      ...
      <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
        <div style={{ padding: '20px 24px', borderBottom: '1px solid var(--border-color)' }}>
          <h2 style={{ fontSize: '16px', fontWeight: '600', margin: 0 }}>Churn Details</h2>
        </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '13px', minWidth: '700px' }}>
            <thead style={{ position: 'sticky', top: 0, backgroundColor: 'var(--bg-card)', zIndex: 10 }}>
              <tr style={{ color: 'var(--text-secondary)' }}>
                ...
                <th style={{ padding: '12px 24px', fontWeight: '500' }}>Reason</th>
                <th style={{ padding: '12px 24px', fontWeight: '500' }}>Churn Month</th>
              </tr>
            </thead>
            <tbody>
              {details.map((d, i) => (
                <tr key={i} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                  <td style={{ padding: '12px 24px', fontWeight: '500' }}>{d.name}</td>
                  <td style={{ padding: '12px 24px' }}>
                    {d.tier && tierColors[d.tier] ? (
                      <span style={{ color: tierColors[d.tier], fontSize: '11px', fontWeight: '600', border: `1px solid ${tierColors[d.tier]}50`, padding: '2px 8px', borderRadius: '4px' }}>{d.tier}</span>
                    ) : <span style={{ color: 'var(--text-secondary)' }}>—</span>}
                  </td>
                  <td style={{ padding: '12px 24px' }}>
                    {d.churn_reason ? (
                      <span style={{ color: categoryColors[d.churn_reason] || '#8B9AB0', fontSize: '12px', fontWeight: '500' }}>
                        {CHURN_CATEGORIES.find(c => c.value === d.churn_reason)?.label || d.churn_reason}
                      </span>
                    ) : <span style={{ color: 'var(--text-secondary)' }}>—</span>}
                  </td>
                  <td style={{ padding: '12px 24px', color: 'var(--text-secondary)' }}>{d.churn_month || '—'}</td>
                </tr>
              ))}
              {details.length === 0 && (
                <tr>
