...
export async function GET(req, { params }) {
  try {
    ...
    // Fetch client_products (new architecture) and payment_history
    const clientProducts = await all('SELECT * FROM client_products WHERE client_id = ? AND is_active = 1 ORDER BY id DESC', [id]);
    ...
    // Transform client_products + payment_history into history-like format for the modal
    // Group payment_history by product_id to get the full picture per product
    const newHistory = [];
    for (const product of clientProducts) {
      const productPayments = paymentHistory.filter(ph => ph.product_id === product.id);

      // Get the latest until_date from payment_history for valid_stopped_date
      const latestUntilDate = productPayments.length > 0 ? productPayments[0].until_date : null;

      // Build history entry that looks like a renewal
      newHistory.push({
        sr_no: `CP_${product.id}`,
        ...
        valid_stopped_date: latestUntilDate || product.valid_until,
        amount_received: productPayments.reduce((sum, ph) => sum + parseAmount(ph.amount), 0).toString(),
        payment_received_date: productPayments.length > 0 ? productPayments[0].date : null,
        reference_no: productPayments.length > 0 ? (productPayments[0].notes || '') : '',
        visual_status: product.is_active === 1 ? 'Active' : '',
        ...
        _payment_history: productPayments,
      });
    }
