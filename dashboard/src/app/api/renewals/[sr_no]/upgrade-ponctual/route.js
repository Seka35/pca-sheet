...
// Upgrades a specific period (sr_no) - can be paid or unpaid.
// The upgrade only affects THAT specific period, not other months.
export async function POST(req, { params }) {
  ...
  try {
    ...
    const { to_tier, to_setup, expires_at, payment_date, amount_received, bank_name, reference_no } = await req.json();

    if (!to_tier && !to_setup) {
      return NextResponse.json({ error: 'to_tier or to_setup is required' }, { status: 400 });
    }

    // Get the specific period to upgrade
    const renewal = get('SELECT * FROM renewals WHERE sr_no = ?', [sr_no]);
    if (!renewal) {
      return NextResponse.json({ error: 'Product not found' }, { status: 404 });
    }

    // Determine the tier/setup to upgrade FROM
    ...
    // Set expires_at to 1 month from payment_date if provided, otherwise 1 month from now
    const expiresDate = expires_at || (() => {
      const d = payment_date ? new Date(payment_date) : new Date();
      d.setMonth(d.getMonth() + 1);
      return d.toISOString().split('T')[0];
    })();

    // Build upgrade chain
    const upgradeDate = payment_date || new Date().toISOString().split('T')[0];
    ...
    // Update ONLY this specific period (sr_no), not siblings
    run(
      `UPDATE renewals SET
        tier = ?,
        setup_type = ?,
