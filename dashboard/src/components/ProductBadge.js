export default function ProductBadge({ tier, setup_type, is_trial, is_ponctual, original_tier, original_setup, showUpgradeBadge, upgradedTier }) {
  const getColors = (type) => {
    ...
    if (t.includes('old setup')) return { bg: 'rgba(249, 115, 22, 0.15)', color: '#FB923C', label: 'old setup' };
    if (t.includes('invincible')) return { bg: 'rgba(249, 115, 22, 0.15)', color: '#FB923C', label: 'Invincible set up (old)' };
